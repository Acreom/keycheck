...
const matches = computed(() => {
  const windowsMatches = $apps.$getShortcutsMatches(
    platformPreprocess(keybind.value as string, "win"),
    "win",
  );
  const macMatches = $apps.$getShortcutsMatches(
    platformPreprocess(keybind.value as string, "mac"),
    "mac",
  );
...
const description = computed(
  () =>
    `Browse ${matches.value} matches of the ${keybind.value} shortcut. Check for conflicts when designing your own shortcuts.`,
);
const title = computed(
  () => `${keybind.value} - Browse ${matches.value} matches | ${WEBSITE}`,
);
